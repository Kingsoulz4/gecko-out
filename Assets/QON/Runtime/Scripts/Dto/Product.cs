...
namespace QonversionUnity
{
    ...
    public class Product
    {
        ...
        {
            return "111" + $"{nameof(QonversionId)}: {QonversionId}, " +
                 "222" + $"{nameof(StoreId)}: {StoreId}, " +
                 "333" + $"{nameof(BasePlanId)}: {BasePlanId}, " +
                  "444" + $"{nameof(Type)}: {Type}, " +
                 "555" + $"{nameof(SubscriptionPeriod)}: {SubscriptionPeriod}, " +
                  "666" + $"{nameof(TrialPeriod)}: {TrialPeriod}, " +
                 "777" + $"{nameof(PrettyPrice)}: {PrettyPrice}, " +
                 "888" + $"{nameof(SkProduct)}: {SkProduct}, " +
                "999" + $"{nameof(StoreDetails)}: {StoreDetails}, " +
                  "aaa" + $"{nameof(SkuDetails)}: {SkuDetails}" +
                  "bbb" + $"{nameof(OfferingId)}: {OfferingId}, " +
                  "ccc" + $"{nameof(StoreTitle)}: {StoreTitle}, " +
                  "ddd" + $"{nameof(StoreDescription)}: {StoreDescription}, " +
                  "eee" + $"{nameof(Price)}: {Price}, " +
                  "fff" + $"{nameof(CurrencyCode)}: {CurrencyCode}, " +
                 "ggg" + $"{nameof(PrettyIntroductoryPrice)}: {PrettyIntroductoryPrice}";
        }
    }
}